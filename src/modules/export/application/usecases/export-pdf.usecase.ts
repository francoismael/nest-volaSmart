  ...
  async execute(userId: string): Promise<Buffer> {
    ...
    return new Promise((resolve, reject) => {
      ...
      for (let i = operations.length - 1; i >= 0; i--) {
        ...
        cumulArr[i] =
          prev + (operations[i].credit ?? 0) - (operations[i].debit ?? 0);
      }
      ...
      const drawPageHeader = () => {
        ...
        doc
          ...
          .text(
            ...
            MARGIN,
            22,
            { align: 'right', width: TABLE_W, ...LB },
          );
        doc
          .fillColor('#6B7280')
          .fontSize(7.5)
          .text(`${operations.length} opération(s)`, MARGIN, 36, {
            align: 'right',
            width: TABLE_W,
            ...LB,
          });
      ...
      const drawTableHeader = (y: number): number => {
        ...
        doc.text('LIBELLÉ', C.label, y + 7, {
          width: C.notes - C.label - 6,
          ...LB,
        });
        doc.text('NOTES', C.notes, y + 7, {
          width: C.debit - C.notes - 6,
          ...LB,
        });
        doc.text('DÉBIT', C.debit, y + 7, {
          width: C.credit - C.debit - 4,
          align: 'right',
          ...LB,
        });
        doc.text('CRÉDIT', C.credit, y + 7, {
          width: C.balance - C.credit - 4,
          align: 'right',
          ...LB,
        });
        doc.text('SOLDE', C.balance, y + 7, {
          width: C.end - C.balance - 4,
          align: 'right',
          ...LB,
        });
        return y + ROW_H;
      ...
      for (let i = 0; i < operations.length; i++) {
        ...
        const notesStr = (op.notes ?? '').substring(0, 12);
        ...
        doc
          .fillColor(balV >= 0 ? '#1D4ED8' : '#DC2626')
          .font('Helvetica-Bold')
          .fontSize(7.5)
          .text(`${balSign}${fmt(balV)}`, C.balance, y + 7, {
            width: C.end - C.balance - 4,
            align: 'right',
            ...LB,
          });
      ...
      doc
        .fillColor('#FCA5A5')
        .fontSize(8)
        .text(fmt(totalDebit), C.debit, y + 9, {
          width: C.credit - C.debit - 4,
          align: 'right',
          ...LB,
        });
      doc.fillColor('#86EFAC').text(fmt(totalCredit), C.credit, y + 9, {
        width: C.balance - C.credit - 4,
        align: 'right',
        ...LB,
      });
      doc
        .fillColor(balance >= 0 ? '#93C5FD' : '#FCA5A5')
        .text(`${balance < 0 ? '-' : ''}${fmt(balance)}`, C.balance, y + 9, {
          width: C.end - C.balance - 4,
          align: 'right',
          ...LB,
        });
